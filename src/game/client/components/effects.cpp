...
void CEffects::Blood(vec2 Pos, vec2 Dir)
{
	...
	if (frandom()*100 < 2)
	{
		...
		b.m_LifeSpan = 8.0f + frandom()*2.0f;
...
void CEffects::Splatter(vec2 Pos, float Angle)
{
	CBloodspill b;
	b.SetDefault();
	b.m_Pos = Pos;
	...
	b.m_LifeSpan = 32.0f + frandom()*8.0f;		
	b.m_Size = 24.0f + frandom()*32;
	
	b.m_Rot = Angle;
	
	b.m_Color = vec4(frandom()*0.3f + 0.5f, 0, 0, 1.0f);
...
void CEffects::Spark(vec2 Pos)
{
	...
	b.m_LifeSpan = 0.1f + frandom()*0.15f;
	b.m_Rotspeed = frandom()*12.0f - frandom()*12.0f;
	
	b.m_Vel = RandomDir() * ((frandom()+0.065f)*500.0f);
