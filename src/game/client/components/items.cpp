...
void CItems::RenderProjectile(const CNetObj_Projectile *pCurrent, int ItemID)
{
	...
	{
		if(pCurrent->m_Type == WEAPON_SHOTGUN && Ct > 0.01f) m_pClient->m_pEffects->BulletTrail(TrailPos1, TrailPos2, vec4(0.5f, 0.5f, 0.2f, 0.75f));
		if(pCurrent->m_Type == WEAPON_GUN) m_pClient->m_pEffects->BulletTrail(TrailPos1, TrailPos2, vec4(0.8f, 0.1f, 0.1f, 0.75f));
...
void CItems::RenderPickup(const CNetObj_Pickup *pPrev, const CNetObj_Pickup *pCurrent)
{
	...
	if (pCurrent->m_Type == POWERUP_WEAPON)
	{
		...
		/*
		if (g_Config.m_GoreCustomWeapons)
		{
			RenderTools()->SelectSprite(WeaponSprite[pCurrent->m_Subtype]);
		}
		else*/
